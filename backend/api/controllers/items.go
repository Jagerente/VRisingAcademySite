...
order by
    ...
    itemtypes.title as typetitle,
    items.knowledgeid,
    array_agg(distinct recipestations.stationid) as stations,
    array_agg(distinct reciperesults.recipeid) as recipes,
    array_agg(distinct recipeingredients.recipeid) as reagentFor,
    array_agg(distinct tags.value) as tags,
    stats.durability,
    stats.gearLevel,
    stats.mainStat,
    items.setid,
    ...
    sets.description,
    stats.slotid,
    array_agg(distinct secondarystats.bonus) as bonusStats,
    array_agg(distinct itemlocations.locationid) as locations,
    array_agg(distinct structurevariants.variantid) as variants,
    array_agg(distinct salvageables.id) as salvageables,
    array_agg(distinct salvageableresults.salvageableid) as salvageableOf
from
    items
    ...
    full join recipestations on recipestations.recipeid = rcp.id
    full join reciperesults on reciperesults.itemid = items.id
    full join itemstats as stats on stats.id = items.id
    full join sets on sets.id = items.setid
    full join itemtags on itemtags.itemid=items.id
    full join tags on tags.id = itemtags.tagid
    full join salvageables on salvageables.itemid = items.id
    full join salvageableresults on salvageableresults.itemid = items.id
    full join itemlocations on itemlocations.itemid = items.id
    full join structurevariants on structurevariants.structureid = items.id
    full join secondaryitemstats on secondaryitemstats.statsid = stats.id
    full join secondarystats on secondarystats.id = secondaryitemstats.secondarystatid
where
    items.id IS NOT NULL
group by
    items.id,
...
func handleListRequest(ctx *gin.Context) {
	var query string = ItemFullQuery
	if ctx.Query("v2") != "" {
		query = ItemFullQueryV2
	}
	...
	for rows.Next() {
		item := models.Item{
			Stations:      make([]sql.NullInt32, 0),
			Recipes:       make([]sql.NullInt32, 0),
			ReagentFor:    make([]sql.NullInt32, 0),
			Tags:          make([]string, 0),
			BonusStats:    make([]sql.NullString, 0),
			Locations:     make([]sql.NullInt32, 0),
			Variants:      make([]sql.NullInt32, 0),
			Salvageables:  make([]sql.NullInt32, 0),
			SalvageableOf: make([]sql.NullInt32, 0),
